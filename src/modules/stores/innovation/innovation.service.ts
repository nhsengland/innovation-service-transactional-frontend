...
export class InnovationService {
  ...
  submitInnovation(innovationId: string): Observable<{ id: string, status: keyof typeof INNOVATION_STATUS }> {
    ...
    return this.http.patch<{ id: string, status: keyof typeof INNOVATION_STATUS }>(url.buildUrl(), {}).pipe(take(1), map(response => response));
  ...
  updateSectionInfo(innovationId: string, sectionKey: string, data: MappedObjectType): Observable<MappedObjectType> {
    ...
    return this.http.put<MappedObjectType>(url.buildUrl(), data).pipe(take(1), map(response => response));
  ...
  submitSections(innovationId: string, sectionKey: string): Observable<any> {
    ...
    return this.http.patch<any>(url.buildUrl(), { sectionKey }).pipe(take(1), map(response => response));
  }
  ...
  upsertSectionEvidenceInfo(innovationId: string, data: MappedObjectType, evidenceId?: string): Observable<{ id: string }> {
    ...
    if (evidenceId) {
      ...
      return this.http.put<void>(url.buildUrl(), { ...data }).pipe(take(1), map(() => ({ id: evidenceId })));
    } else {
