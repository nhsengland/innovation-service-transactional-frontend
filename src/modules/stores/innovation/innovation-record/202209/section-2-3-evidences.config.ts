...
type StepPayloadType = Omit<BaseType, 'id' | 'files' | 'createdBy' | 'createdAt' | 'updatedBy' | 'updatedAt'> & {
  files: { id: string; name: string; url: string }[];
};

type OutboundPayloadType = Omit<StepPayloadType, 'files'> & { files: string[] };

type SummaryPayloadType = Omit<StepPayloadType, 'files'> & {
      ...
      parameters: [
        {
          ...
          description:
            'Please provide a short summary including the scope of the study and the key findings. Accessors will read this summary to understand if any particular piece of evidence is of interest in relation to what they can help you with.',
