        ...
        parameters: [
          {
            ...
            description:
              'This means something that performs or demonstrates the same function that the final product or service would.',
            validations: { isRequired: [true, 'Choose one option'] },
...
function runtimeRules(steps: WizardStepType[], currentValues: StepPayloadType, currentStep: number | 'summary'): void {
  ...
  if (currentValues.categories?.length === 0) {
    ...
    const selectedCategories = mainCategoryItems.filter(
      category => currentValues.categories?.some(e => e === category.value)
    );
        ...
        parameters: [
          {
            ...
            description:
              'Your innovation may be a combination of various categories. Selecting the primary category will help us find the right people to support you.',
            validations: { isRequired: [true, 'Choose one option'] },
      ...
      parameters: [
        {
          ...
          description:
            "We're asking this so that we can find the organisations and people who are in the best position to support you.",
          items: areasItems
        }
      ]
    }),
      ...
      parameters: [
        {
          ...
          description:
            'Select up to 5 options. Your answer will help us to establish your primary point of contact if you choose to sign up for the Innovation Service.',
...
function inboundParsing(data: InboundPayloadType): StepPayloadType {
  return {
    ...
    location:
      data.countryName &&
      locationItems
        .filter(item => !['', 'Based outside UK'].includes(item.value))
        .map(item => item.value)
        .includes(data.countryName)
        ? data.countryName
        : 'Based outside UK'
  };
}
...
function summaryParsing(data: StepPayloadType): WizardSummaryType[] {
  ...
  if (data.categories && data.categories.length > 1) {
    toReturn.push({
      label: stepsLabels.l6,
      value:
        data.otherMainCategoryDescription || mainCategoryItems.find(item => item.value === data.mainCategory)?.label,
      editStepNumber: editStepNumber++
    });
  }
