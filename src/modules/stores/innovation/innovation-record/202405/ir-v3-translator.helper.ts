...
import { InnovationSectionInfoDTOv3 } from './ir-v3-types';
  ...
  answer.forEach((value: any) => {
    ...
    if (item) {
      ...
      if (schemaQuestion.addQuestion && (schemaQuestion.dataType === 'checkbox-array')) {  
        const conditionalAnswer = searchAnswer(subSection, schemaQuestion.addQuestion);

        const item2 = conditionalAnswer.items.find((item: any) => item.id === IRV3Helper.camelize(value.addQuestion));
        schemaAnswers[`${conditionalAnswer.id}|${itemId}`] = item2.id;
        data[`${conditionalAnswer.id}|${itemId}`] = item2.id;
      }
    } else {
...
export class IRV3Helper {
  ...
  static translateIR(innovationRecord: MappedObjectType): InnovationSectionInfoDTOv3 {
    ...
    return {
      ...
      section: IRV3Helper.camelize(innovationRecord.section),
