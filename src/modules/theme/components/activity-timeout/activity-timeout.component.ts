...
export class ActivityTimeoutComponent implements OnInit, OnDestroy {
  ...
  check(): void {
    ...
    if (difference < 60000) {
      // Shows warning if 1 minute remaining.
      this.state = 'WARNING';
      this.warningCounter = this.secondsToWarningCounter;

      if (this.warningCounterSubscription) {
        this.warningCounterSubscription.unsubscribe();
      }
  ...
  setAccessibility(): void {
    if (isPlatformBrowser(this.platformId) && !this.firstElement) {
      setTimeout(() => {
        ...
        this.lastElement?.addEventListener('blur', () => {
          this.firstElement?.focus();
        });
      });
    }
