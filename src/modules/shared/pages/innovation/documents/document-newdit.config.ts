...
export const WIZARD_BASE_QUESTIONS: WizardEngineModel = new WizardEngineModel({
  steps: [
    new FormEngineModel({
      parameters: [{
        id: 'name', dataType: 'text', label: stepsLabels.l4.label, description: stepsLabels.l4.description,
        validations: { isRequired: [true, 'Name is required'], maxLength: 100 }
      }]
    }),
    new FormEngineModel({
      parameters: [{
        id: 'description', dataType: 'textarea', label: stepsLabels.l5.label, description: stepsLabels.l5.description,
        lengthLimit: 's'
      }]
    }),
    new FormEngineModel({
      parameters: [{
        id: 'file', dataType: 'file-upload', label: stepsLabels.l6.label, description: stepsLabels.l6.description,
...
export const WIZARD_WITH_LOCATION_QUESTIONS: WizardEngineModel = new WizardEngineModel({
  steps: [
    new FormEngineModel({
      parameters: [{
        id: 'relatedWithSection', dataType: 'radio-group', label: stepsLabels.l1.label,
  ...
  showSummary: true,
  runtimeRules: [(steps: WizardStepType[], currentValues: StepPayloadType, currentStep: number | 'summary') => wizardWithLocationRuntimeRules(steps, currentValues, currentStep)],
...
function wizardWithLocationRuntimeRules(steps: WizardStepType[], data: StepPayloadType, currentStep: number | 'summary'): void {
