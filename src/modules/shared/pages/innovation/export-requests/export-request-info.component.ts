...
export class PageInnovationExportRequestInfoComponent extends CoreComponent implements OnInit {
    ...
    this.innovationsService.updateExportRequestStatus(this.innovationId, this.requestId, { status }).subscribe({
      next: () => {
        ...
        if (status === InnovationExportRequestStatusEnum.CANCELLED) {
          this.setRedirectAlertSuccess(`You've cancelled this request to export the innovation record`, { width: '2.thirds' });
        }

        this.redirectTo(`/${this.stores.authentication.userUrlBasePath()}/innovations/${this.innovationId}/record/export-requests/list`)
  ...
  rejectRequestRedirect() {
    if (!this.isInnovatorType) { return; }
    this.redirectTo(`/${this.stores.authentication.userUrlBasePath()}/innovations/${this.innovationId}/record/export-requests/${this.innovationRequest?.id}/reject`);
  }
