...
export class PageInnovationSupportSummaryProgressUpdateDeleteComponent extends CoreComponent implements OnInit {
  ...
  deleteHistoryItem(): void {
    ...
    this.innovationsService.deleteSupportSummaryProgressUpdate(this.innovationId, this.supportSummaryHistoryId).subscribe({
      next: () => {
        this.setRedirectAlertSuccess('Your progress update has been deleted from the support summary');
        this.redirectTo(this.baseUrl);
      },
      error: () => this.setAlertError('Unable to delete support summary item. Please try again or contact us for further help')
    });
