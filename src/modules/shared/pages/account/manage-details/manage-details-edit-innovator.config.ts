        ...
        items: [
          {
            value: 'YES',
            label: 'Yes',
            conditional: new FormEngineParameterModel({ id: 'organisationName', dataType: 'text', label: 'Enter the company or organisation name', validations: { isRequired: true, maxLength: 100 } })
          },
          { value: 'NO', label: 'No' }
        ]
      }]
...
function outboundParsing(data: StepPayloadType): OutboundPayloadType {
  ...
  return {
    ...
    organisation: {
      ...
      registrationNumber: (data.hasRegistrationNumber === 'YES' && data.organisationRegistrationNumber) ? data.organisationRegistrationNumber : null
    }
  };
...
function summaryParsing(data: StepPayloadType): WizardSummaryType[] {
  ...
  toReturn.push(
    ...
    { label: 'Is company or organisation?', value: data.isCompanyOrOrganisation === 'YES' ? `Yes, ${data.organisationName}` : 'No', editStepNumber: 5 }
  );
  ...
  if (data.isCompanyOrOrganisation === 'YES') {
    ...
    toReturn.push(
      ...
      { label: 'Company UK registration number', value: data.hasRegistrationNumber === 'YES' ? `Yes, ${data.organisationRegistrationNumber}` : 'No', editStepNumber: 8 },
    );
