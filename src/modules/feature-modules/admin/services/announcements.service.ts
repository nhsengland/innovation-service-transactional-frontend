...
export class AnnouncementsService extends CoreService {
  ...
  getAnnouncementsList(queryParams: APIQueryParamsType<{}>): Observable<GetAnnouncementsListType> {
    ...
    return this.http.get<GetAnnouncementsListType>(url.buildUrl()).pipe(take(1), map(response => response));
  ...
  getAnnouncementInfo(announcementId: string): Observable<GetAnnouncementInfoType> {
    ...
    return this.http.get<GetAnnouncementInfoType>(url.buildUrl()).pipe(take(1), map(response => response));
  ...
  updateAnnouncement(announcementId: string, status: AnnouncementStatusEnum, data: UpsertAnnouncementType): Observable<void> {
