...
import { GetAnnouncementInfoType, UpsertAnnouncementType } from '@modules/feature-modules/admin/services/announcements.service';
...
const stepsLabels = {
  ...
  l3: {
    ...
    description: 'Add more detailed information to guide the user. For example, if you think this organisation will be able to support you, you can share your innovation with them in your data sharing preferences.'
  },
  l4: {
...
export const ANNOUNCEMENT_NEW_QUESTIONS: WizardEngineModel = new WizardEngineModel({
  ...
  steps: [
    new FormEngineModel({
      parameters: [{
        id: 'title', dataType: 'text', label: stepsLabels.l1.label, description: stepsLabels.l1.description,
        validations: { isRequired: [true, 'Title is required'], maxLength: 100 }
      }]
    }),
    new FormEngineModel({
      label: stepsLabels.l2.label, description: stepsLabels.l2.description,
      parameters: [
    ...
    new FormEngineModel({
      parameters: [{
        id: 'userRoles', dataType: 'checkbox-array', label: stepsLabels.l6.label, description: stepsLabels.l6.description,
...
function outboundParsing(data: StepPayloadType): OutboundPayloadType {
  ...
  return {
    title: data.title,
    userRoles: data.userRoles,
    startsAt: DatesHelper.parseIntoValidFormat(data.startsAt ?? null, 'yyyy-MM-dd') ?? new Date().toISOString().slice(0, 10),
    ...
    params: {
      ...(data.insetTitle || data.insetContent || data.insetLinkLabel ? {
        inset: {
          ...
          ...(data.insetLinkLabel && data.insetLinkUrl ? { link: { label: data.insetLinkLabel, url: data.insetLinkUrl } } : {})
        }
      } : {}),
      ...(data.content ? { content: data.content } : {}),
      ...(data.actionLinkLabel && data.actionLinkUrl ? { actionLink: { label: data.actionLinkLabel, url: data.actionLinkUrl } } : {})
    }
  };
