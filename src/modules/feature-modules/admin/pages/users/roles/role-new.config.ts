...
function wizardRuntimeRules(steps: WizardStepType[], data: StepPayloadType): void {
  ...
  if (data.role === UserRoleEnum.ASSESSMENT) {
    ...
    if (data.organisationId && !data.inboundData.unitId) {
      ...
      const units = (organisation?.units ?? []).filter(u => !userUnits.has(u.id)).map(u => ({ value: u.id, label: u.name }));
...
function inboundParsing(data: InboundPayloadType): StepPayloadType {
  ...
  const isAccessor = data.userRoles.some(r => r.role === UserRoleEnum.QUALIFYING_ACCESSOR || r.role === UserRoleEnum.ACCESSOR);
