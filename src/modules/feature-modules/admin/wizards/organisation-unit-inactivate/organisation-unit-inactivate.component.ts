...
export class WizardOrganisationUnitInactivateComponent extends CoreComponent implements OnInit {
  ...
  onSummaryStepIn(): void {
    this.wizard.setStepData<SummaryWithConfirmStepInputType>('summaryStep', {
      ...
      confirmCheckbox: {
        label: `I understand that when this unit is inactivated, if the organisation has no more units, it will also be inactivated and not be seen on the Innovation Service platform`
      },
  ...
  onSubmit(stepData: WizardStepEventType<SummaryWithConfirmStepOutputType>): void {
    ...
    this.adminOrganisationsService
      .inactivateOrganisationUnit(this.wizard.data.organisation.id, this.wizard.data.organisationUnit.id)
      .subscribe({
        next: () => {
