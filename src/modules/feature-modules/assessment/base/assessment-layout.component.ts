...
export class AssessmentLayoutComponent extends CoreComponent {
  ...
  constructor(
    ...
    this.navigationMenuBar = {
      leftItems: [{ title: 'Home', link: '/assessment/dashboard' }],
      rightItems: [
        {
          title: 'Innovations',
  ...
  private onRouteChange(event: NavigationEnd): void {
    ...
    if (currentRouteInnovationId) {
      innovation = RoutingHelper.getRouteData(
        this.activatedRoute
      ).innovationData;
    }
    ...
    switch (this.layoutOptions.type) {
      case 'userAccountMenu':
        this.leftSideBar = [
          { title: 'Your details', link: `/assessment/account/manage-details` },
          {
            ...
            link: `/assessment/account/manage-account`,
          },
        ];
        break;
      ...
      case 'innovationLeftAsideMenu':
        this.leftSideBar = [
          {
            title: 'Overview',
            link: `/assessment/innovations/${currentRouteInnovationId}/overview`,
          },
          {
            title: 'Innovation record',
            link: `/assessment/innovations/${currentRouteInnovationId}/record`,
          },
          // { title: 'Action tracker', link: `/assessment/innovations/${currentRouteInnovationId}/action-tracker` },
          {
            title: 'Comments',
            link: `/assessment/innovations/${currentRouteInnovationId}/comments`,
          },
        ];
        if (innovation.status === 'IN_PROGRESS') {
          this.leftSideBar.push({
            title: 'Support status',
            link: `/assessment/innovations/${currentRouteInnovationId}/support`,
          });
        }

        this.leftSideBar.push({
          title: 'Activity log',
          link: `/assessment/innovations/${currentRouteInnovationId}/activity-log`,
        });
