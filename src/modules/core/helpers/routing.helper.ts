...
export class RoutingHelper {
  ...
  static getRouteParams(route: ActivatedRoute): ActivatedRouteSnapshot['params'] {
    return {
      ...
      ...route.children.reduce(
        (acc: ActivatedRouteSnapshot['params'], child: ActivatedRoute) => ({
          ...RoutingHelper.getRouteParams(child),
          ...acc
        }),
        {}
      )
  ...
  static getRouteData<T>(route: ActivatedRoute): T {
    return {
      ...
      ...route.children.reduce(
        (acc: T, child: ActivatedRoute) => ({ ...RoutingHelper.getRouteData<T>(child), ...acc }),
        {} as T
      )
    };
  }
