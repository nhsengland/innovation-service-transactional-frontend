import { MappedObjectType } from '../interfaces/base.interfaces';
...
const searchAnswer = (
  v2Answers: MappedObjectType,
  question: MappedObjectType
) => {
  if (v2Answers[question.id]) return v2Answers[question.id];

  switch (question.id) {
    case "officeLocation":
      return v2Answers.countryName &&
        question.items.some((item: any) => item.label === v2Answers.countryName)
...
class IRV3Helper {
  ...
  static translateIR(innovationRecord: MappedObjectType) {
    const v3Answers: MappedObjectType = {};
    const data: MappedObjectType = {};

    const v2Answers = Object.keys(innovationRecord.document).reduce(
      (acc, key) => {
