...
export class InnovationOverviewComponent extends CoreComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin([
    ...
    ]).subscribe(([innovation, sectionSummary, innovationSupportsList]) => {
      ...
      const occurrences = (innovation.supports ?? []).map(item => item.status)
        .reduce((acc, status) => (
          acc[status] ? ++acc[status].count : acc[status] = { count: 1, text: this.translate('shared.catalog.innovation.support_status.' + status + '.name') }, acc),
